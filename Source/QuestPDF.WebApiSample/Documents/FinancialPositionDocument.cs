...
/// <summary>
/// Financial Position (Balance Sheet) Document
/// </summary>
public class FinancialPositionDocument : LedgerReportDocument
{
    private new FinancialPositionModel Model { get; }

    public FinancialPositionDocument(FinancialPositionModel model) : base(model)
    {
        Model = model;
    }
    ...
    protected override void ComposeContent(IContainer container)
    {
        container.PaddingTop(20).Column(column =>
        {
            ...
            column.Item().Row(row =>
            {
                ...
                {
                    col.Item().Text($"Total Assets: {Model.TotalAssets:N2}").FontSize(12).SemiBold();
                    col.Item().Text($"Total Liabilities: {Model.TotalLiabilities:N2}").FontSize(12).SemiBold();
                    col.Item().Text($"Total Equity: {Model.TotalEquity:N2}").FontSize(12).SemiBold();
                    col.Item().PaddingTop(5).Text($"Assets = Liabilities + Equity").FontSize(14).Bold().FontColor(Colors.Blue.Darken3);
                    col.Item().Text($"{Model.TotalAssets:N2} = {Model.TotalLiabilities:N2} + {Model.TotalEquity:N2}").FontSize(12).SemiBold();
                });
            });

...
/// </summary>
public class TrialBalanceDocument : LedgerReportDocument
{
    private new TrialBalanceModel Model { get; }

    public TrialBalanceDocument(TrialBalanceModel model) : base(model)
    {
        Model = model;
    }
    ...
    protected override void ComposeContent(IContainer container)
    {
        container.PaddingTop(20).Column(column =>
        {
            ...
            column.Item().Row(row =>
            {
                ...
                {
                    col.Item().Text($"Total Debit: {Model.TotalDebit:N2}").FontSize(12).SemiBold();
                    col.Item().Text($"Total Credit: {Model.TotalCredit:N2}").FontSize(12).SemiBold();
                    col.Item().PaddingTop(5).Text($"Difference: {Math.Abs(Model.TotalDebit - Model.TotalCredit):N2}").FontSize(14).Bold()
                        .FontColor(Model.TotalDebit == Model.TotalCredit ? Colors.Green.Darken3 : Colors.Red.Darken3);
                });
            });
            ...
            column.Item().Table(table =>
            {
                ...
                {
                    // First header cell
                    header.Cell().Element(HeaderStyle).Text("Account").SemiBold();

...
/// </summary>
public class IncomeStatementDocument : LedgerReportDocument
{
    private new IncomeStatementModel Model { get; }

    public IncomeStatementDocument(IncomeStatementModel model) : base(model)
    {
        Model = model;
    }
    ...
    protected override void ComposeContent(IContainer container)
    {
        container.PaddingTop(20).Column(column =>
        {
            ...
            column.Item().Row(row =>
            {
                ...
                {
                    col.Item().Text($"Total Revenue: {Model.TotalRevenue:N2}").FontSize(12).SemiBold();
                    col.Item().Text($"Total Expenses: {Model.TotalExpenses:N2}").FontSize(12).SemiBold();
                    col.Item().PaddingTop(5).Text($"Net Income: {Model.NetIncome:N2}").FontSize(14).Bold().FontColor(Colors.Green.Darken3);
                });
            });

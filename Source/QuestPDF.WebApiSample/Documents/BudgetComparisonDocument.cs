...
/// <summary>
/// Budget Comparison Report Document
/// </summary>
public class BudgetComparisonDocument : ComparisonReportDocument
{
    private new BudgetComparisonModel Model { get; }

    public BudgetComparisonDocument(BudgetComparisonModel model) : base(model)
    {
        Model = model;
    }
    ...
    protected override void ComposeContent(IContainer container)
    {
        container.PaddingTop(20).Column(column =>
        {
            ...
            column.Item().Row(row =>
            {
                ...
                {
                    col.Item().Text($"Budget Amount: {Model.BudgetAmount:N2}").FontSize(12).SemiBold();
                    col.Item().Text($"Actual Amount: {Model.ActualAmount:N2}").FontSize(12).SemiBold();
                    col.Item().PaddingTop(5).Text($"Budget Variance: {Model.BudgetVariance:N2}").FontSize(12).SemiBold();
                    col.Item().Text($"Variance Percentage: {Model.BudgetVariancePercentage:F2}%").FontSize(12).SemiBold();
                });
            });
